...
# Server-side image store: image_id -> numpy array
image_store = {}

# Video loop control
_video_loop_stop = False
...
def store_image(img):
    """Store image server-side and return its ID."""
    img_id = str(uuid.uuid4())[:8]
    image_store[img_id] = img
    return img_id
...
def upload_image():
    ...
    filepath = os.path.join(UPLOAD_FOLDER, file_id + ext)
    file.save(filepath)
...
def upload_video():
    ...
    # Store first frame in memory for preview
    image_store[file_id] = frame
...
def stop_video_loop():
    ...
    global _video_loop_stop
    _video_loop_stop = True
...
# Store: script_id -> { path, node_id, last_content }
_script_sessions = {}
SCRIPT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'scripts')
os.makedirs(SCRIPT_DIR, exist_ok=True)


@app.route('/api/script/open', methods=['POST'])
def open_script_editor():
    """Write script to temp .py file and open it in VS Code or Notepad."""
    data = request.json
    node_id = data.get('nodeId', 'unknown')
    script = data.get('script', '')
    editor = data.get('editor', 'vscode')  # 'vscode' or 'notepad'

    # Create/reuse script file
    script_id = node_id
    filename = f'node_{node_id}.py'
    filepath = os.path.join(SCRIPT_DIR, filename)

    # Write header comment + script content
    header = (
        "# ──────────────────────────────────────────────\n"
    ...
    _script_sessions[script_id] = {
        'path': filepath,
        'node_id': node_id,
        'last_content': header + script,
    }
    ...
    return jsonify({
        'scriptId': script_id,
        'filepath': filepath,
        'message': f'Opened in {"VS Code" if editor == "vscode" else "Notepad"}',
    })


@app.route('/api/script/read', methods=['POST'])
def read_script_file():
    """Read the current content of the script file (after external editing)."""
    data = request.json
    script_id = data.get('scriptId', '')
    node_id = data.get('nodeId', '')

    # Try to find existing session
    session = _script_sessions.get(script_id)

    # Fallback: try to find file by nodeId convention
    if not session and node_id:
        fallback_path = os.path.join(SCRIPT_DIR, f'node_{node_id}.py')
        if os.path.exists(fallback_path):
            session = {'path': fallback_path, 'node_id': node_id}

    if not session:
        return jsonify({'error': 'Script session not found. Open the editor first (VS Code or Notepad).'}), 404
    ...
    # Strip the header comment (everything before double newline after header)
    # Find the end of the header block
    header_end_marker = "# ──────────────────────────────────────────────\n\n"
    idx = content.rfind(header_end_marker)
    if idx >= 0:
        script_body = content[idx + len(header_end_marker):]
    else:
        script_body = content
